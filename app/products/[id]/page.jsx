...
const ProductDetailPage = ({ params }) => {
  ...
  return (
    <section className='w-[90%] md:w-[80%] mx-auto py-10'>
      <div className='flex flex-col lg:flex-row gap-10'>
        ...
        <div className='lg:w-1/2 flex flex-col items-center'>
          <div className='relative w-full h-[400px] md:h-[500px] overflow-hidden rounded-lg shadow-md mb-4'>
            <Image
              src={mainImage || product.image[0]} // Fallback to first image if mainImage not set
              alt={product.name}
              fill
              style={{ objectFit: 'contain', objectPosition: 'center' }} // 'contain' for detail view
          ...
          {product.image && product.image.length > 1 && (
            <div className='flex gap-3 justify-center'>
              {product.image.map((img, index) => (
                <div
                  key={index}
                  className={`relative w-20 h-20 cursor-pointer border-2 ${img === mainImage ? 'border-blue-600' : 'border-gray-200'} rounded-md overflow-hidden`}
        ...
        <div className='lg:w-1/2 flex flex-col gap-4'>
          ...
          <div className='flex gap-2 text-sm text-gray-500'>
            <p>Category: <Link href={`/products?category=${product.category}`} className="font-semibold text-blue-600 hover:underline">{product.category}</Link></p>
            {product.subCategory && <p> | Subcategory: <Link href={`/products?subCategory=${product.subCategory}`} className="font-semibold text-blue-600 hover:underline">{product.subCategory}</Link></p>}
          </div>
      ...
      <RelatedProduct category={product.category} subCategory={product.subCategory} currentProductId={product._id}/>
    </section>
  );
};
